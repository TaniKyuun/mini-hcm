...
export function useWeeklySummary(startDate?: string) {
	...
	const [resolvedStartDate, setResolvedStartDate] = useState<string | null>(
		null,
	);
	...
	const refresh = useCallback(async () => {
		...
		try {
			...
			setDays(result.days);
			setResolvedStartDate(result.startDate);
		} catch (caught) {
	...
	return { days, startDate: resolvedStartDate, loading, error, refresh };
}
